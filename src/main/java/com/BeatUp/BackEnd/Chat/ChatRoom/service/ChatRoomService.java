...
public class ChatRoomService {
    ...
    public RoomResponse createRoom(UUID createdBy, CreateRoomRequest request){
        ...
        // 3. 중복 방 생성 방지(선택적)
        Optional<ChatRoom> existingRoom = chatRoomRespository
        ...
        ChatRoom savedRoom = chatRoomRespository.save(chatRoom);
    ...
    private void validateSubjectId(String type, UUID subjectId){
        boolean exists = switch (type){
            case "MATCH" -> matchGroupRespository.existsById(subjectId);
